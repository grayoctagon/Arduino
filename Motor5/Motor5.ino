...
void loop(){
  ...
  if(currentMill >= previousMillis + (1000/printsPerS)) {
   
  if(voltage != 0 || ((movingSpeedSum/movingSpeedNum)) > 0.01) 
    Serial.println(String("")
        //+"time:" + String(currentMill)
        + ",voltage:" + String(getVolts()) 
        + ",amps:" + String(getAmpsReal(ampsSum/ampsNum)*100) 
        + ",mps:" + String((movingSpeedSum/movingSpeedNum)*100) 
        + ",loopCountPerS:" + String(loopCount*printsPerS) 
        + ",positionMM:" + String(positionMM)
        + ",targetPositionMM:" + String(targetPositionMM)
        + ",voltRAW:" + String(voltage)
        + ",tmps:" + String(targetSpeed*100)
        );
    ampsSum = 0;
    ampsNum = 0;
    movingSpeedSum=0;
    movingSpeedNum=0;
    previousMillis = previousMillis + (1000/printsPerS);
    loopCount = 0;
  }
  //delay(100);
  //Serial.println("time:" + String(millis()/100.0));
}

float speedVolt = 0;

void applyVoltage() {
  float diffMM = diff(targetPositionMM,positionMM);
  if(diffMM > 1) {
    float maxVolt=12;
    float minVolt=0.1;
    targetSpeed = 0.2;
    float minSpeed = targetSpeed*0.15;
  
  // maximale be-/nd-schleunigung festsetzen
  // TODO: movingDirSpeed


  // abbremsen am ende
  if(diffMM < 50) {
    targetSpeed = targetSpeed*(diffMM/50);
...
if(speedVolt < minVolt) {
  speedVolt = minVolt;
}
if(speedVolt > maxVolt) {
  speedVolt = maxVolt;
}

    if(positionMM > targetPositionMM){
      setVolts(speedVolt);
      return;
    } else {
      setVolts(speedVolt*-1);
      return;
    }
  }
  setVolts(0);
...
void setVolts(float volts) {
  voltage=volts/12.0*255;
  if(voltage<-255)voltage=-255;
  if(voltage>255)voltage=255;

  if(voltage>0){
    analogWrite(motorA,0);
    analogWrite(motorB,voltage);
  }else{
    
    analogWrite(motorA,voltage*-1);
...
void checkInput() {
  ...
  if(c == 'a') {
    Serial.println("a");
    targetPositionMM  = targetPositionMM + 30;
  }
  ...
  if(c == 'y') {
    Serial.println("y");
    targetPositionMM  = targetPositionMM + 120;
  }
...
float getVolts() {
  return voltage*12.0/255.0;
}

float getAmpsReal(long amps) {
  return (512.0-amps)/512*30;
}
